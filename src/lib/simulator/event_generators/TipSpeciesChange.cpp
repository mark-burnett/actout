...
double
FixedRateTipSpeciesChange::rate(State const* state,
...
StateModifications
FixedRateTipSpeciesChange::perform_event(State* state,
    ...
    update_filament_tip(state->filaments[*iter].get());
...
void
FixedRateTipSpeciesChange::update_cache(State const* state,
        StateModifications const& modifications) {
...
void
FixedRateTipSpeciesChange::update_cache_deleted_filament(
        StateModifications const& modifications) {
...
void
FixedRateTipSpeciesChange::update_cache_created_filaments(State const* state,
        StateModifications const& modifications) {
    for (auto& filament_id : modifications.created_filaments) {
        if (old_species_ ==
                peek_filament(state->filaments[filament_id].get())) {
...
void
FixedRateTipSpeciesChange::update_cache_modified_filaments(State const* state,
        StateModifications const& modifications) {
