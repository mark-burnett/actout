...
SingleStrandFilament::SingleStrandFilament(uint64_t const& num_species,
    ...
    while (begin != end) {
        auto boundary = std::adjacent_find(begin, end,
                std::not_equal_to<SpeciesMap::species_index_t const>());
        if (boundary != end) {
            ++boundary;
        }
...
SingleStrandFilament::SingleStrandFilament(uint64_t const& num_species,
        uint64_t const& number, SpeciesMap::species_index_t const& species) {
    initialize_counts(num_species);
...
SingleStrandFilament::fracture_segment(std::list<Segment>::iterator& segment,
        uint64_t const& index, SpeciesMap::species_index_t const& new_species) {
    if (1 == segment->number) {
